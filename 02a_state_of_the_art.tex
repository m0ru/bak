...
architecture}, in it's full form, requires one to define the following four functions and pass them to Elm's \texttt{Html.program} (that runs the app):

\begin{enumerate}
    \item \texttt{init : (Model, Cmd Msg)}, which defines the basic app-state. It's the first \texttt{update}, but before any \texttt{Msg}s have been produced by the user, if you so will.
    \item the function \texttt{update : Msg -> Model -> (Model, Cmd Msg)} that performs the same role as \texttt{reduce} in Redux, with \texttt{Msg}s in Elm being the same as actions in Redux. The second part of the returned tupple, the \texttt{Cmd} allows to request side-effect-full computation -- e.g. generating random numbers or asynchronous operations like HTTP-request -- that are otherwise impossible due to Elm's guarantees of side-effect-freeness (i.e. purity). The results of the \texttt{Cmd} are fed back as
        \texttt{Msg} to the next update.
    \item \texttt{subscriptions : Model -> Sub Msg} that allow to set up additional sources for \texttt{Msg}s beside user-input, e.g. listening on a websocket.
    \item and lastly, \texttt{view : Model -> Html Msg} to produce the HTML from the model.
\end{enumerate}

Note, that \texttt{model : Model} (similar to \texttt{init}), \texttt{update : Msg -> Model -> Model} and \texttt{view} are the essential ones of of these, if you you start the app with \texttt{Html.beginnerProgramm}.
