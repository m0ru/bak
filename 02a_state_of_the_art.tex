...
\begin{comment}
  * virtual dom
    * don't need to think of intermediate states / state transitions, treat each update as if the page were rendered entirely anew / only need to define mapping from data to 
  * jsx
  * react native
  * one-way data flow
  * atomic components
  ...
  * better performance with immutable data as tracking changes is as easy as comparing the object id
\end{comment}
